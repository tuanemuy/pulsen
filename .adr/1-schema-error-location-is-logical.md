# ADR: スキーマ走査で見つかる設定・定義エラーの「位置」は論理位置で示す
...
spec は「config.yaml がパース不能(構文エラー・未知キー)ならエラー位置を表示して非0で終了する」ことを全コマンド共通の縮退挙動として求める。`.adr/1-yaml-value-then-hand-written-schema-walk.md` に従い YAML は `Value` 化してから手書きでスキーマを走査するため、行・列が取れるのは構文解析までで、`Value` になった後のスキーマ違反(未知キー・型不一致)には行・列が残らない。
...
- スパンを保持する別の YAML クレートへ差し替える — `.adr/1-yaml-value-then-hand-written-schema-walk.md` の選定基準(重複キー検出・エラー位置・エラー種の区別)を同時に満たすものは実測で `serde_yaml_ng` だけだった
