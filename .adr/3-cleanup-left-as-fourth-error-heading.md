...
ADR-098 は `errors` を「タスクファイルに何を残したか」で3つの見出し(失敗を記録 / 起動の結果が未確定 / スキップ)に分けた。
...
- cron 運用では tick のサマリーが唯一の窓であり(ADR-092)、見出しの語義と帳簿の状態が食い違うと運用者の読みが外れる
...
本 ADR は ADR-098 の見出しの数(3分類 → 4分類)と語義(帳簿に何を残したか → 報告が何を残したか)を置き換える。ADR-098 の本文は判断が下された時点の記録として書き換えない。
