# ADR: run ディレクトリの各ファイルは JSON で書き、マーカーは空ファイルにする
...
タスクファイルと同じく `serde_json` の DTO で書く（`.adr/1-dependency-selection.md` / `.adr/1-task-file-json-and-corrupt-classification.md` の作法をそのまま適用する）。
