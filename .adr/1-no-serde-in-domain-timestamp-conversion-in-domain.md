# ADR: ドメイン型に serde を実装せず、`Timestamp` の RFC3339 変換はドメインに持たせる
...
タスクファイル(JSON)とグローバル設定・ワークフロー定義(YAML)の読み書きが必要になる。ドメイン型に `#[derive(Serialize, Deserialize)]` を付ければ実装は短くなるが、`pulsen-domain` が serde に依存する(`.adr/1-domain-crate-workspace.md` に反する)。加えて spec は「デコード(YAML/JSON → ドメイン型)はアダプター境界の責務」であり、復号の失敗を `Corrupt` / `SnapshotUnreadable` に**区別して**写像することを求めている。
...
- TC-port-clock-002 は「`now` の返値を RFC3339 に直列化して再パースし、元の `Timestamp` と等価」を要求するが、`pulsen-domain` のみに依存する適合スイート(`.adr/1-domain-crate-workspace.md`)からは `pulsen::util` を呼べず、このケースが書けない
...
- **`Timestamp` の表現可能範囲を 0001-01-01T00:00:00Z 〜 9999-12-31T23:59:59Z に閉じる**。`format!("{:04}", y)` は5桁年で単に桁が増え、20文字固定の `parse_rfc3339` が受け付けないため、範囲を閉じないと `to_rfc3339 ∘ parse_rfc3339 = id` が全域で成り立たない(実測確認)。範囲外の壁時計を `Clock` がどう扱うかは `.adr/1-infallible-ports-absorb-failure-at-construction.md`
- この結果、`time` クレートは不要になる(`.adr/1-dependency-selection.md`)。`util::rfc3339` モジュールは設けない
