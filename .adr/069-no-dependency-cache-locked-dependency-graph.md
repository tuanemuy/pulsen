# 069: 依存のキャッシュは入れず、`--locked` で依存グラフを固定する
...
1. `Swatinem/rust-cache` — 事実上の標準だが第三者製 Action(ADR-065 の方針に反する)
...
- ワークフローが短く、赤の原因がキャッシュの陳腐化である可能性を最初から排除できる。第三者製 Action がゼロという ADR-065 の方針が保てる
- `--locked` により、CI が検証する依存グラフが `Cargo.lock` と厳密に一致する。ADR-066 の MSRV 検証は「このロックファイルの依存グラフが宣言した版でコンパイルできる」という意味になる。ロックファイルの更新漏れは全ジョブの即時失敗として現れる
