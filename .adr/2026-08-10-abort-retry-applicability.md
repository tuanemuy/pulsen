# ADR: abort は全域・retry は stopped 専用とする
...
requirements の初版では abort は launching / running(`.adr/2026-08-10-abort-pending-task.md` で pending を追加)を対象とし、failed(リトライ待ち)や completed(遷移待ち)のタスクを止められるかが未定義だった。retry も適用対象の実行状態が明示されていなかった。failed のタスクは次のtickで自動リトライされるため、「これ以上リトライさせたくない」を表現する手段が要る。

## 前提

- abort は「タスクの停止(実行中なら kill を伴う)」に一般化済み(`.adr/2026-08-10-abort-pending-task.md`)
