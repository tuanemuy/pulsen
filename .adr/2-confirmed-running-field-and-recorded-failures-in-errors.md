# ADR: 起動確認だけをサマリーの新しいフィールドにし、記録した失敗は `errors` に載せる
...
集計されないまま tick を終えるとサマリーが空になり、「処理対象のタスクはありませんでした。」と表示される（`.adr/2-empty-summary-means-nothing-to-process.md` の判定）。主経路である起動確認が毎回この表示になるため、放置できない。
...
`errors` の形も spec の `{ task_id, path, message }` ではなく分類の列挙として持つ（`.adr/2-tick-errors-are-structured-values.md`）。文言の組み立ては `cli::render` に置く。
...
- 書き込みを行った tick が必ずサマリーに現れ、`.adr/2-empty-summary-means-nothing-to-process.md` の不変が構成として成立する
