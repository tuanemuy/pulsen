# 068: `ProcessController` は自バイナリのパスと同定情報の取得元を構築時に注入される
...
- 取得機構失敗の再現は同型で、**存在しないパスを `identity_source` として構築した2つ目のコントローラ**を `failing_identity_controller` フックが返す。ADR-067 の写像表により、どのプラットフォームでもこの構成は `Err(Io)` に落ちる
...
- この注入点は、ADR-067 の三値化（不在 / 機構失敗の区別）が正しく実装されたことを照合側（`starttime_of`）に対して検証するときの足場にもなる
