...
- `home` — `std::env::home_dir()` は Windows の挙動が Rust 1.85.0 で修正され、非推奨は 1.87.0 で解除された。宣言している MSRV 1.89(ADR-022)はこれより後なので、`-D warnings` でも警告にならない(本環境の 1.97.1 で実測確認)。std で足りるものを外部クレートにする理由がない。ホーム解決は合成ルート1箇所(ADR-031)
