# ADR: `clippy::wildcard_enum_match_arm` はドメインクレートにのみ適用する
...
（`pulsen` はのちに `unsafe_code` を `deny` にするため workspace lints の継承をやめた。`.adr/2-windows-detach-suppresses-handle-inheritance.md`）
