...
- 決定時点の `spec/testcases/ports/task-repository.md` は破損フィクスチャを旧文面「スナップショットフィールドのみを**構文不正**な内容に置き換える」と書いており、この設計では「有効な JSON だがスナップショットとして解釈不能」と読むほかなかった。Issue #9 で spec 側が後者の語に追従して解消した
