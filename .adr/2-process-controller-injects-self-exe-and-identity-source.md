# ADR: `ProcessController` は自バイナリのパスと同定情報の取得元を構築時に注入される
...
`SystemProcessController::new(self_exe: PathBuf, identity_source: IdentitySource, clock: SystemClock)` とし、実行ファイルのパスと**同定情報の取得元**を構築時に注入する。`std::env::current_exe()` を読むのは合成ルート（`cli::wire`）の1箇所だけにする — `.adr/1-git-cli-shell-out-and-target-classification.md` が `git_program` を、`.adr/1-pulsen-home-layout-in-application-layer.md` が環境の読み取りを合成ルートに閉じたのと同型。
...
- `SpawnError` の再現は、**存在しないパスを `self_exe` として構築した2つ目のコントローラ**をハーネスの `failing_controller` フックが返す形にする（`.adr/1-git-cli-shell-out-and-target-classification.md` / `.adr/1-port-conformance-suite-and-harness-hooks.md` の `failing_manager` と同型）。本番のインスタンスはイミュータブルなまま、権限操作にも root 実行の可否にも依存しない
- 取得機構失敗の再現は同型で、**存在しないパスを `identity_source` として構築した2つ目のコントローラ**を `failing_identity_controller` フックが返す。`.adr/2-process-controller-without-unsafe.md` の写像表により、どのプラットフォームでもこの構成は `Err(Io)` に落ちる
...
- この注入点は、`.adr/2-process-controller-without-unsafe.md` の三値化（不在 / 機構失敗の区別）が正しく実装されたことを照合側（`starttime_of`）に対して検証するときの足場にもなる
