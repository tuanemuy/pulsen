# ADR: グローバルホームのレイアウトはアプリケーション層に置く
...
当初は `PulsenHome`(`config_path` / `workflows_dir` / `state_root` / `worktree_root` / `lock_path` の導出)を `adapter::home` に置き、`application` がそれを import する構成だった。これはアプリケーション層がアダプターを直接参照することになり、依存が内 → 外になる。`.adr/1-domain-crate-workspace.md` がクレート境界で守ろうとしている性質がクレート内で緩む。
