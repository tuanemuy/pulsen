# 070: ジョブは fmt 1つ + OS マトリクス2組に分け、clippy は test ジョブへ同居させる
...
CI が回すのは `build` / `test` / `clippy --all-targets -- -D warnings` / `fmt --check` / MSRV で、いずれも 3 OS のマトリクスが要る(ADR-066)。分割の粒度に選択肢がある。

1. 1ジョブに全部を詰める — マトリクスにすると fmt が3回走る。どのコマンドで落ちたかはログを追えば分かるが、PR のチェック一覧では見えない
2. コマンドごとにジョブを分ける(build / test / clippy を別ジョブ)— チェック一覧は読みやすいが、clippy ジョブが依存を丸ごと再コンパイルする(キャッシュを入れない以上、これは純粋な重複。ADR-069)
...
- `msrv`(3 OS): `cargo build --all-targets` のみ(ADR-066)
