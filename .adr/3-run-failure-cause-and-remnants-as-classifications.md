...
- `errors` の分類は `cli::render` の網羅 `match` が受け、文言は表示側が組む(ADR-081)
- ポート機構の失敗は単一の `Io` 文字列で表される(ADR-086)ため、そこから先は分類できない
...
- **完成文言を文字列で運ぶ(従来の形)** — ADR-081 の規約の外に2変種が残り、失敗の主体を取り違えた文言が実際に出ていた。テストも文言でしか主張できない
...
- `errors` の全変種が ADR-081 の規約に揃い、規約の外にある変種が1つも無くなる
