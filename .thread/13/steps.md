...
- **対象ファイル:** `.adr/13-holder-capability-skip-vs-fail.md`(新規) / `.thread/13/adr.md` / `crates/pulsen/tests/common/lock.rs` / `crates/pulsen/tests/conformance_lock.rs` / `crates/pulsen-conformance/HOOKS.md`
- **変更内容:**
  ...
  2. 昇格分を `.adr/13-holder-capability-skip-vs-fail.md` として起票する。既存の最大採番は **072**(`ls .adr/` で確認済み)なので連番の起点は 073。書式は `.adr/038` に従い、見出しを `## ステータス` / `## コンテキスト` / `## 決定` / `## 検討した代替案` / `## 影響`、ステータス語を **承認済み** に変換する。
