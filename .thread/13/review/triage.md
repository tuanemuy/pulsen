...
| Key | 初出 | 元ID | 判定 | 理由 | 再指摘 |
| --- | --- | --- | --- | --- | --- |
| 作業ログと手順書に旧断定が残る | 004 | code W-001 | fix | 正本 `.adr/073` とコードは否定側へ直っており、`.thread/13/` 側だけ「温まったあとは異常」と断定していた | 0 |
| 手順に --no-fail-fast が無い | 004 | docs W-001 | fix | 5件が2バイナリに分かれるため、打ち切りで期待結果（5件の失敗）が観測できない | 0 |

## 5周目

| Key | 初出 | 元ID | 判定 | 理由 | 再指摘 |
| --- | --- | --- | --- | --- | --- |
| PR 本文の出典が旧 run | 005 | final W-001 | fix | `.thread/13/` 側だけ最終のコード変更コミットの run に寄せたため、同じ AC-10 に2つの run が並んでいた | 0 |
