...
`.github/workflows/ci.yml` を新規追加し、3ランナーで実行して**全7ジョブが緑**になった（run 31666626824、コミット `e524981` = 最後にコードとワークフローが変わったコミット。以降の差分はこのメモを含むドキュメントのみ）。steps.md のステップ1〜11 が完了し、ステップ12 の機械確認と、PR 本文・Issue #10・PR #11 への記録も済んでいる。
...
| 31657976822 | `af24360` | 全7ジョブ緑 | フィクスチャの可搬化（adr.md ADR-011）と `write_atomic` の再試行（adr.md ADR-010・ADR-012） |
| 31661056619 | `2c99c1b` | 全7ジョブ緑 | 前提検査への `id` 追加 / 非 root アサートの fail-open 解消 / `--no-fail-fast` / 除外件数の表示 / MSRV の `env` 経由 / 読み取りを `read_atomic` に通す（adr.md ADR-013） |
...
| 31666626824 | `e524981` | 全7ジョブ緑 | 予算のテストが押さえていない配線を、テスト名とコメントから外した |

スキップの集合（unix 1件 / Windows 11件）とテストバイナリの本数は、緑になった6つの run すべてで同一である。
...
**この CI が実測したのは `e524981`（本 PR の変更をすべて適用した時点）のコードであり、PR #11 が追加するプロセス同定・デタッチ起動の Windows 挙動は含まれない。** Issue #10 のクローズは「クロスプラットフォームが検証済み」を意味しない。この事実は PR 本文と [Issue #10 のコメント](https://github.com/tuanemuy/pulsen/issues/10#issuecomment-5275112376) に残してある。
