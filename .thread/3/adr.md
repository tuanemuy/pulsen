...
**採番を掛けるのは、順序の契約がポートをまたぐメソッドだけにする。** 本スライスでは4つ — `TaskRepository::save` / `TaskRepository::save_degraded` / `CommandRunner::run` / `ProcessController::try_kill_remnants`。
...
- `try_kill_remnants` を含めるのは、spec の手続きD 3 が `DiedWithoutExit` に対して `try_kill_remnants` → `fail_run` → `save` を契約として定めており、順序が `ProcessController` と `TaskRepository` をまたぐためである。`save` を先に置く実装は、それぞれのポートの列だけを見るかぎり期待どおりに見える
...
**猶予のうちに消えなければ強い終了へ昇格する。** POSIX は `-TERM` → `-KILL` の2段。

**昇格の段を持つのは、捕捉できる終了と捕捉できない終了が別の操作であるプラットフォームに限る。** 両者が同じ操作に落ちるプラットフォーム(Windows の `taskkill /T /F`)では2段目を起動しない — 同じ操作の再実行は結果を変えず、猶予ぶん排他ロックを保持し、同定子が pid そのものであるぶん pid 再利用による誤殺の窓をもう一度開くだけである。どちらであるかはアダプターの `terminate` モジュールが `ESCALATES` として宣言する。
