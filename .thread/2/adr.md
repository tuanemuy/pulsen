...
採番は `.adr/` の続き番号。`.adr/` への昇格判定は片付けフェーズで行い、昇格しても番号は変わらない。Status の直下に `.adr/` のファイル名を持つエントリが昇格済み。ADR-074 / 083 / 084 / 087 / 088 / 101 は本スライス限りの作業判断か他のエントリに吸収される派生であり、昇格しない — `.adr/` 側では 074 / 083 / 084 / 087 / 088 が欠番になる。

先頭のエントリだけ番号が飛ぶのは、本スライスが 073 として起票したあとに main で別の決定が `.adr/073` として昇格し、番号が衝突したため。番号は詰めない規約なので、衝突した側を最大番号の続き(101)へ動かした。本文の順は起票順のまま置く。

## ADR-101: tick の分岐は全実行状態を網羅する `match` で書き、本スライス外の手続きは配線しない
...
ADR-101 は tick の分岐を全実行状態の網羅 `match` として書くと決めた。素直に `match task.execution() { ... }` と書くと、アームの中でタスクを消費する手続き(`launch` / `confirm_spawn` はいずれも `self` を消費する遷移関数を呼ぶ)へ渡せない — 走査対象の借用が `match` 全体で生きるため。
...
spec の出力DTO は `launched` / `transitioned` / `skipped_back` / `frozen` / `notified` / `archived` / `errors` / `gc_deleted` / `gc_errors` の9フィールドで、ADR-101 はこれをそのまま持つと決めた。ところが本スライスで配線した書き込み経路のうち、次の4つはどのフィールドにも集計されない。
