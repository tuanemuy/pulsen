...
AC-1 の「OS 依存分岐がアダプター層に隔離されている」ことの機械的確認。Issue #1 の `cfg(unix)` / `cfg(windows)` だけを見る grep では ADR-003 が生む `#[cfg(target_os = "linux")]` を素通りさせるため、述語を4つに広げる。
...
- **確認ポイント:** サマリーに `launched` / `frozen` / `errors` などの空フィールドが並ばず、「対象なし」だけが読めること（ADR-001: 値の入っていないフィールドは表示しない）。2回目の実行で表示が変わらないこと。
...
- **確認ポイント:** `workspace.path` と `current_attempt.run_dir` がいずれも**絶対パス**であること（外部スケジューラーからの起動が成立する前提。AC-12）。`pid` の中身が `{"pid":<数値>,"kill_ident":"<非空文字列>"}` であること — `kill_ident` が空文字や `0` になっていると Issue #3 の kill が無関係なプロセスを対象にしうる（ADR-003）。`starttime` が `{"ident":"...","wall":"..."}` の形で、`ident` が非空であること。
...
- **確認ポイント:** `current_attempt.process.starttime.ident` が `attempt-1/starttime` ファイルの `ident` と**完全に一致**すること — ラッパーが記録した値がそのまま帳簿へ移ることが、Issue #3 の生存判定の前提になる（ADR-003）。取り込み後も `attempt_count` が 0 のままであること（起動は attempt_count を消費しない）。
...
- **期待結果:**
  ...
  - 手順4: `{"code":0}`（ADR-008 により JSON。素の `0` ではない）。
...
- **確認ポイント:** 手順4 の run_dir は `state/runs/<task-id>/attempt-<n>` の形をしているだけで、**タスクが実在しなくても動く**こと（ラッパーは帳簿を読まない。ADR-006）。ラッパー自身の標準出力には何も出ないこと（結果はファイルにしか現れない）。ラッパー自身の終了コードは spec が規定していないので主張しない。
...
- **確認ポイント:** `running` のタスクに対して tick が「未実装」等の報告を出さないこと（ADR-001: 未配線のアームは報告もしない）。サマリーの `errors` が空であること。
...
- **期待結果:**
  ...
  - 手順7: 以降の tick は T12 に何もしない（`updated_at` が変わらない。ADR-001 で `Stopped` のアームは未配線）。
  - 手順8: T12 の通知行は無い（notify は Issue #3。ADR-002）。
- **確認ポイント:** `notified_at` が `null` のまま永続化されていること（Issue #3 がマージされた後の最初の tick が catch-up するための at-least-once の前提。ADR-002）。失敗しても runディレクトリが作られないこと（採番は worktree 確保の**後**）。
...
- **対応する受け入れ基準:** AC-12
- **前提:** フィクスチャA（config 復元済み）。`task-execution.md` TC-20 手順1〜4。plan.md の読み替えにより、手順1 に `pipeline` のタスク（T20p）を1件追加する — `draft.yaml` の T20h は Pending × Cleanup で、`Cleanup` のアームは本スライスで配線しないため（ADR-001）「他タスクへの影響なし」を裏付ける対象にならない。
...
- **期待結果:**
  ...
  - 手順7: T20H は `pending` のまま何も起きない（`Cleanup` のアームが未配線。ADR-001。TC-20 の「アーカイブされる」は Issue #6 に読み替え）。
...
- **確認ポイント:** 手順3 の run_dir は実在するディレクトリだが `<state_root>/runs/<task-id>/attempt-<n>` の形をしていないため拒否されること（ADR-006 の逆写像が `None` を返す経路）。エラーが標準エラーに出て、標準出力が汚れないこと。

## 既存機能への影響確認

- **`pulsen add` の経路が壊れていないこと:** 本スライスは `wire::Runtime` にアクセサとポートを追加し（steps.md ステップ17）、`WireError` に1変種を足す（ADR-004）。`add` は `ProcessController` を必要としないので、`current_exe()` の失敗で `add` が落ちないことを確認する。`.thread/1/testing.md` の確認項目1・2・4（未初期化ホームの案内 / ホーム解決の優先順位 / 登録成功時の表示とタスクファイルの中身）をスポットチェックとして再実行し、Issue #1 時点と同じ結果になること。

- **`pulsen --help` の表示:** サブコマンドとして `add` / `tick` / `help` が並び、`wrapper` が現れないこと（ADR-005）。`pulsen tick --help` に引数が無いこと（`--home` は global フラグとして現れる）。引数の使い方の誤りが clap 既定の exit code 2 になること。

- **AC-1 の grep 期待値の更新:** Issue #1 の testing.md は `crates/pulsen/src/` 側のヒットを `util/atomic.rs` だけとしていた。本スライスで `adapter/process.rs` が加わるため、期待値は2ファイルになる（ADR-003 Consequences）。`crates/pulsen-domain/` が0件であることは変わらない。この期待値の変更は Issue #1 の testing.md 側には反映しない（各 Issue の testing.md はその時点の期待を書く）。
