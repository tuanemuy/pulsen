...
- **手続きB(終端処理)・手続きE(gc)** と、それらが使う `WorktreeManager::remove` / `RunStore::{list_runs, delete_attempt, remove_task_dir_if_empty}` / `GcPolicy` / `RemoveOutcome` — Issue #6。tick の分岐は網羅 `match` として書くが、`Cleanup` のアームは配線しない(adr.md ADR-101)。
