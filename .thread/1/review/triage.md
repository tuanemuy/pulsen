...
6周目のレビューは Blocker 0 / Warning **2** だったが、メインが台帳へ転記する際に W-001 を落とし、W-002（HOOKS.md）だけを R6-W-001 として記録していた。7周目のレビューが同じ指摘を再度挙げたことで判明した。落ちていた指摘を以下に記録する。

| ID | Key（照合キー） | 判定 | 理由 | 再指摘回数 |
|----|----------------|------|------|-----------|
| R6-W-003 | conformance/exclusive_lock.rs TC-002 の非ブロッキング判定 | fix | TC-002 は保持プロセスを起こした後、呼び出しスレッドでそのまま `try_acquire()` を呼ぶため、解放を待つ実装に当てるとスイート全体がハングする。隣接する TC-003 の doc が同じ危険を名指しで説明し ADR-060 で解いているのに、TC-002 だけ適用漏れだった。2周目 R2-W-026・3周目 R3-W-002 と同じ失敗モードの最後の1箇所。現行アダプターは適合しているため本 PR のテストは緑で、効くのは AC-8 が約束する「別実装に同じスイートを当てる」場面 | 1（2周目 R2-W-026） |

反映: TC-003 の期限監視を `attempt_while_held` として共通化し、TC-002 も同じ経路を通す形にした。`Lock: Sync` の要求はこの2ケースに閉じ、ハーネス全体には課さない（ADR-060 のまま）。

検証: `FileExclusiveLock::try_acquire` を `File::lock()`（解放を待つ実装）に変異させて実測。修正前は `cargo test --test conformance_lock` が 300 秒返らなかったのに対し、修正後は **5.01 秒で TC-002・TC-003 が揃って FAILED**。変異は復元済み。
