...
各エントリの Status は正本への昇格状況の索引を兼ねる。`.adr/` のファイル名を持つものは昇格済み、ADR-041 / 047 は既存の `.adr/` エントリに本文を反映済みで単独のファイルを持たない。ADR-056〜059 は欠番で、本書にもエントリを持たない — 採番したまま使わなかった番号であり、決定が失われたわけでも別の場所に反映されたわけでもない。番号は詰めず、次の起票は最大番号の続きから行う。
...
- `home` — `std::env::home_dir()` は Windows の挙動が Rust 1.85.0 で修正され、非推奨は 1.87.0 で解除された。宣言している MSRV 1.89（ADR-022）はこれより後なので、`-D warnings` でも警告にならない（本環境の 1.97.1 で警告なしにコンパイル・動作することを実測確認）。std で足りるものを外部クレートにする理由がない。ホーム解決は合成ルート1箇所（ADR-031）
...
- 良い点: 125行すべてが「ポートのみ28件 / フック85件 / spec が明示するスキップ可12件」のいずれかに埋まり、フックが spec 由来であることを対応表で確認できる。フックの形が「対象アクセサ + 意味だけを受け取るフック + 別ハンドル」の3種に揃う
...
spec/domains/task.md は `TaskFilePath` を「レイアウトの単一の定義箇所（アダプターも同じ導出を使う）」と定めているが、持っているのは `active` / `archived`（ファイル1件のパス）だけだった。`TaskRepository` の走査（`list_active` / `list_archived`）は**IDを持たない状態でディレクトリ**を必要とし、さらに「命名形式（`<task-id>.json`）に合致するエントリのみを対象とする」フィルタを必要とする。アダプターが `state_root.join("tasks")` と `.json` の除去を自前で書くと、レイアウトの定義箇所が2つになる。

### Decision

`TaskFilePath` に `active_dir` / `archived_dir` / `file_name(id)` / `parse_file_name(name) -> Option<TaskId>` を足し、`active` / `archived` をこれらの合成として定義する。アダプターはディレクトリもフィルタもこの4つ経由で得る。`.json` という拡張子はドメイン側の定数のまま（spec のパス導出がすでに拡張子を含む）で、**中身の形式**（JSON であること・キー構成）はアダプターの責務（ADR-025）という分担を保つ。

### 検討した代替案

- アダプターにディレクトリ名と拡張子の定数を持たせる — レイアウトの定義が2箇所になり、`TaskFilePath` を置いた理由（show の保存先パス表示・monitoring の直接閲覧が同じ導出に乗ること）が崩れる
- `TaskFilePath::active(...).parent()` で導出する — ディレクトリを得るためだけにダミーのIDが要る

### Consequences

- 良い点: 走査の命名形式フィルタが `TaskId::parse` の帰結として定義され、一時ファイル残骸（`.tmpXXXX`）が「先頭が英数字でない」ことで自動的に外れる。後続スライスの RunStore も同じ形（`attempt-<n>` の判定をドメインに置く）で書ける
...
ADR-025 は「`save_degraded` は `snapshot` の生バイト列をそのまま書き戻す」ことと「タスクファイルは整形して人間が読める状態にする」ことの両方を求める。`snapshot` を常に `Box<RawValue>` として持つと、正常タスクの保存でもスナップショットが1行に潰れる（serde_json は生値を整形せずそのまま書き出す）。逆に常に構造化した DTO にすると、解釈できないスナップショットを書き戻せない。

### Decision

`TaskFileDto<Snapshot>` を型引数1つで抽象化し、`Task` の符号化は `TaskFileDto<SnapshotDto>`（整形される）、`DegradedTask` の符号化と全ての復号は `TaskFileDto<Box<RawValue>>`（生のまま）を使う。タスク側フィールドの定義・`deny_unknown_fields` は1箇所のままになる。

### 検討した代替案

- タスク側フィールドを持つ構造体を2つ書く — 同じ11フィールドとその検証が二重化し、片方だけ直す事故を生む
- 常に生値で持ち、正常タスクの保存時に整形済みテキストを埋め込む — 埋め込み位置のインデントが合わず、「人間可読」の意図が中途半端になる

### Consequences

- 良い点: 正常タスクのファイルは全体が整形され、破損したスナップショットはバイト単位で温存される。復号経路が1本のまま
...
`record_bytes` / `snapshot_bytes` は「提供できなければスキップ」というフックであり、`require!` で受けるのが枠組みの既定の書き方になっている。しかし TC-port-task-repository-004 / 009 はこの2つを**操作の前後で2回**呼ぶ。後半の呼び出しを `require!` にすると、「操作がファイル（スナップショット）を消したせいで観測できなくなった」場合にケースが失敗ではなくスキップになり、契約違反が緑で通る（実測: `save_degraded` がスナップショットを捨てる実装にしても TC-009 が通った）。

### Decision

スキップの判定は**前提条件を用意する時点の1回だけ**にする。同じフックを操作後の観測に使うときは `require!` を使わず、`Some(期待値)` との等値比較として書く。フックが前半で `Some` を返している以上、後半の `None` は環境の制約ではなく実装の失敗である。
...
spec/testcases/task/register-task.md の異常系は「config.yaml がパース不能（構文エラー・未知キー）→ **エラー位置**を表示して非0で終了する」を求める。ADR-021 に従い YAML は `Value` 化してから手書きでスキーマを走査するため、行・列が取れるのは構文解析までで、`Value` になった後のスキーマ違反（未知キー・型不一致）には行・列が残らない。受け入れテストが「位置」を何に対して確かめるのかを決める必要がある。
...
ただしワークフロー定義のスキーマ違反（`UnknownKey` / `InvalidValue`）は絶対パスを伴わず、論理位置だけで示す。解決先を知っているのはストアのアダプターだけで、パスを載せられる場所が自由形式のメッセージに限られるため（ADR-054）。
...
- 良い点: 利用者は壊れている箇所をキー単位で特定できる。エラー種の区別（ADR-021）を保ったまま案内が具体になる
- トレードオフ: スキーマ違反では行番号が出ないため、大きな設定では絞り込みが1段階弱い。spec の「エラー位置」を行・列に限定しない読みで満たしている点は、spec 側への注記として提起する
...
spec の異常系は「`workflow:` キーがなく、ファイル名由来の表示名が不正になるパス（拡張子を除くと空になるファイル名等）」を求める。しかし Rust の `Path::file_stem` は `.yaml` のような先頭ドットのファイル名を「拡張子なし・語幹 `.yaml`」として返すため、「拡張子を除くと空」の状態はファイル名からは作れない。`WorkflowName::parse` が拒むのは空文字列と前後の空白の2つである。

### Decision

語幹が空白1文字になるファイル名（` .yaml`）を置いて再現する。ファイルは実在し、内容は有効な定義にする。

### Consequences

- 良い点: 「定義は読み込めたが表示名だけが決まらない」という spec の処理フロー上の位置（load のあとの display_name）を実際に通る。ファイル不在や YAML 不正では通らない経路である
- トレードオフ: 到達するのは `NameError::Empty` ではなく `SurroundingWhitespace` になる（spec の「等」の範囲として扱う）
...
受け入れテスト62件はすべて実バイナリを起動する。各テストが `Command` を直接組み立てると、環境変数 `PULSEN_HOME` の除去（実行環境の設定に結果を左右されないため）やカレントディレクトリの指定を書き漏らしたテストだけが環境依存でフレーキーになる。ロックを保持する別プロセス（ADR-032）の起動手順は、適合テスト（`conformance_lock.rs`）と異常系の受け入れテストの両方が必要とする。

### Decision

- `pulsen add` の起動を `tests/common` のビルダー1つに閉じ、環境変数の除去を既定の振る舞いにする。テストは「ホーム・ワークフロー・リポジトリ・`--base`・カレントディレクトリ」を意図として指定するだけにする
- ロック保持プロセスの起動・解放は `tests/common/lock.rs` に置き、適合テストのハーネスもここを使う
...
`CaseOutcome::report` はスキップを標準出力に書くが、libtest は成功したテストの出力を握り潰す（stderr も同様に捕捉するため `eprintln!` にしても変わらない）。結果として「125件 PASS」の意味が環境で静かに変わる。

権限操作のフックは「制限が実際に効いたことを確認してから `Some` を返す」（ADR-027）ため、root 実行では権限を前提とする8件がスキップになる。許容をプラットフォーム（`cfg(unix)`）で決めると、この環境ではスキップが失敗に変わり、「環境が前提を作れない行はスキップして理由を残す」という運用と食い違う。件数だけの宣言にも、想定した行の代わりに別の行がスキップしても合計が合えば緑になる穴がある。

### Decision

スイートを適用するテストファイルは、スキップを許容するケースの集合を宣言する（`SkipBudget`）。集合の外のスキップはそのケース自身の失敗にし、集合の中のスキップも `SKIP` 行として必ず出力する。宣言の要素はケース名の接頭辞となる TC ID。集合はプラットフォームではなく環境の能力（`permission_restrictions_effective`）から実行時に決め、宣言は `LazyLock` で1度だけ評価する。

### Consequences

- 良い点: 環境が変わって走る行が減れば、その差が失敗として現れる。root 実行・Windows では区分 C の行がスキップとして記録され、plan.md / ADR-027 の運用がそのまま成立する
- 良い点: 同じ probe を受け入れテスト側のスキップ集計にも使えるため、root への態度が2箇所で割れない
- トレードオフ: 宣言が「件数」より長くなる。区分 C の行を足すたびに宣言に加える必要がある
...
TC-port-exclusive-lock-003 は「保持中の `try_acquire` が解放を待たずに返る」ことを主張する。経過時間の判定を取得と同じスレッドで行うと、解放を待つ実装では `try_acquire` が返るまで判定に到達せず、ケースが名指しする失敗モードでハングする（保持の解放はさらに後にある）。ケースが宣言したアサーションで検出できない。

取得を別スレッドに置くには、ロックへの共有参照をスレッドへ渡すため `Lock: Sync` が要る。ADR-027 は TC-042 / TC-044 で同じ制約に当たったとき、スキップ可能なフック（`concurrent_repo`）で `Sync` を隔離した。

### Decision

TC-003 は取得の試行を `thread::scope` の子スレッドで走らせ、親が期限まで完了を監視する。期限を超えたら先に `release_holder` して試行を返らせ、「待った」ことを失敗として報告する。`Sync` はこのケース関数の `where` 境界（`Harness::Lock: Sync`）に閉じ、ハーネストレイトには課さない。ガードは子スレッドの中で落とし、結果だけを値としてスレッド境界を越えさせる（`LockGuard` は `Send` を要求しない）。

TC-042 / TC-044 と違ってスキップ可能なフックにはしない。フックにすると、この行が「待つ実装」に対して失敗ではなくスキップになり、検出したい失敗モードがそのまま素通りする。

### Consequences

- 良い点: 待つ実装に対してケースがハングではなく失敗として現れる。期限超過時も保持を解放してから合流するため、テストが止まらない
- トレードオフ: ExclusiveLock の適合スイートを適用する実装には `Sync` が要る（満たさない実装は境界エラーとして即座に分かる）
...
`PulsenHome::worktree_root()` は本スライスのどこからも呼ばれない。同じ「呼び出しが0の `pub`」として `Runtime::home()` を削除した判断（1周目 W-011）があり、両者の扱いが割れて見える。`pub` は dead_code 検査に掛からないため、基準が無いと「後で使う」を理由に無限に残る。

### Decision

呼び出しの無い `pub` を残すのは、**ADR-031 がホームのレイアウトとして列挙し、構築時に値として検証している**項目に限り、why を添えたときだけ。`worktree_root` は `WorktreeRoot::parse` による絶対性の検証が `PulsenHome::new` の一部で、その検証結果を取り出す口が無ければ検証だけが宙に浮く。`Runtime::home()` は結線の途中結果でこの条件を満たさないため落とした。基準は `application/home.rs` のモジュール doc に置き、後続スライスから見える形にする。

### Consequences

- 良い点: 「残す / 落とす」が値の検証の有無で機械的に決まり、スライスをまたいで同じ判断になる
- トレードオフ: 条件を満たす項目は使われないまま残る。worktree を作るスライスが入るまで、その正しさはユニットテスト（配置の固定）でしか裏付けられない
...
ADR-052 は実バイナリの起動を `tests/common` のビルダーに閉じ、`PULSEN_HOME` の除去を既定にした。しかしホーム解決は3段（`--home` > `PULSEN_HOME` > `~/.pulsen/`）で、既定の段は `HOME` / `USERPROFILE` を読む。ビルダーはこの2つを、`user_home()` を明示したテストでしか差し替えない。

そのため `add(...).run()` とだけ書いたテストは開発者の実 `~/.pulsen/` を解決先にする。config.yaml があればそこにタスクを1件作り、無ければ「未初期化」を返す。どちらも一時ディレクトリの外に出る。既定の段を踏むテストは本 PR で初めて入り、後続スライスでも増える。

### Decision

起動のたびに一時ディレクトリを作り、`HOME` / `USERPROFILE` を既定でそこへ向ける（`PULSEN_HOME` の除去と同じ扱い）。`user_home()` はその上書きとして残す — 既定の段を観測するテストは、解決先のホームを覗くためにパスを必要とする。

3段のどこを踏んでも一時ディレクトリの外に出ないため、ホームの指定を書き忘れても実ホームには届かない。

### Consequences

- 良い点: 「実ホームに触れない」がテストごとの規約ではなくビルダーの不変条件になる。既定の段の経路を後続スライスが増やしても安全側に倒れる
- 良い点: 手動テスト手順も同じ形（`HOME=$(mktemp -d)` を前置き）に揃う
- トレードオフ: 起動1回につき一時ディレクトリを1つ作る。生存期間を子プロセスの終了まで保つ必要がある
...
TC-port-task-repository-042 / 044 は、読み手のスレッドを `AtomicBool` で回し続けながら書き手が `save` / `archive` を行う。読み手の停止条件はこのフラグだけで、フラグを倒すのは `thread::scope` のクロージャ本体の末尾にある文だった。

書き手が `save` / `archive` の失敗でパニックすると、その文に到達しないまま巻き戻る。`thread::scope` は巻き戻しの前に子スレッドを合流させるため、読み手が回り続けてプロセスが返らない。`save` / `archive` が失敗するのはアダプターの欠陥・ディスク満杯といった、まさにスイートが検出すべき状況であり、そこで判定が出ずに固まる。後続スライスが in-memory 実装に同じスイートを適用する場面（AC-8）では、これが CI のタイムアウトになる。ADR-060 がロック TC-003 で扱ったのと同じ失敗モードが、停止フラグの形で残っていた。

### Decision

停止フラグを倒す責務を、スコープを抜けるときに必ず走る `Drop`（`StopOnDrop`）へ移す。正常終了でも巻き戻しでも読み手は止まり、パニックはそのままケースの失敗として現れる。

観測回数を数える位置は変えない — 停止の前に数えるという要件（止めた後の観測は書き込みと重なっていない）は、ガードがスコープの末尾で落ちることで保たれる。

同じ形の並行観測は `util/atomic.rs` のユニットテストにもあり、そちらにも同じガードを置く。
