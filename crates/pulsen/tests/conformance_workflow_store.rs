...
//! 組み立てる。テストが並列に走ってもプロセス全体の状態を壊さない。
...
///
/// 「読み取れない定義ファイル」を作れない環境 — 権限操作を持たないプラットフォーム・
/// root 実行・権限を持たないファイルシステム — では TC-port-workflow-store-030 が走らない。
const PERMISSION_CASES: [&str; 1] = ["tc_port_workflow_store_030"];
