...
`spec/testcases/ports/*.md` のうち、これまでのスライスで扱った10ポート198行を、どう組み立てるかで分類した表。フックは spec の前提条件から導く（`.adr/1-port-conformance-suite-and-harness-hooks.md`）。行を足す・フックを足すときはこの表も更新する。冒頭の集計だけでなく、各節の「残り N 行」も台帳から数え直す。
