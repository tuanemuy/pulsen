...
/// - **消失と到達不能**: その領域にもう無いエントリ(`archive` による移動を含む。走査では
///   列挙した後に消えたもの)は結果から落とす — 失敗ではない。エントリが残ったまま内容へ
///   到達できない場合は落とさず `Corrupt` として報告する(修復の入口を消さない)。この区別は
///   `find` と走査で同じであり、片方だけが「無い」と答えることはない
