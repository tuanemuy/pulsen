...
生成元: spec/domains/(最終同期: 2026-08-11)
...
| DOM-definition-007 | InputText 値オブジェクト | spec/domains/definition.md#名前系文字列-newtype | 制約なしの文字列newtypeとして`{input}`へ渡る展開結果を保持する |
...
| DOM-definition-023 | CommandLine 値オブジェクト | spec/domains/definition.md#commandline | `CommandTemplate::expand`の結果としてのみ生成される1トークン以上のトークン列(先頭がプログラム) |
...
| DOM-definition-049 | RegistrationValidator ドメインサービス | spec/domains/definition.md#registrationvalidator | `validate(def,config)->Result<WorkflowSnapshot,Vec<RegistrationError>>`として全AgentRunステータスの実効エージェント解決・config.agents存在・テンプレート検証・skill_input要否・model要否を最初の1件で打ち切らず全件検証する |
...
| DOM-definition-052 | WorkflowStore.load ポートメソッド | spec/domains/definition.md#workflowstore | `Name`は`<home>/workflows/<n>.yaml`固定解決(`.yml`フォールバックなし)、`Path`はカレントディレクトリから解決し`LoadedWorkflow{parsed,resolved_from}`を返す契約 |
...
| DOM-task-013 | RunDirPath 値オブジェクト | spec/domains/task.md#rundirpath | `derive(state_root,id,n)`で`<state_root>/runs/<task-id>/attempt-<n>`を決定的に導出し、導出結果をタスクファイルにも記録する |
...
| DOM-task-024 | FailureKind 値オブジェクト | spec/domains/task.md#failurenote | `WorktreeCreate`/`WorktreeRemove`/`ArchiveMove`/`SpawnFail`/`JudgeFail`の5種の直和型 |
...
| DOM-task-042 | Task.record_tool_failure ドメイン関数 | spec/domains/task.md#振る舞い遷移関数 | `Pending/Failed`前提でworktree作成/削除・アーカイブ移動失敗を`attempt_count+=1`で記録し超過なら`Stopped{RetryLimitExceeded}`、でなければ`Failed`にする |
...
| DOM-task-053 | TransitionError エラー型 | spec/domains/task.md#エラー型 | `InvalidState`/`WorkspaceAlreadySet`/`WorkspaceNotSet`/`NotAgentRunStatus`/`InvariantViolated`の5種を区別する |
...
| DOM-execution-004 | JudgeOutcome 値オブジェクト | spec/domains/execution.md#judgeoutcome-judgeconclusion | `Completed`/`Failed`/`Skipped`の3値直和型(Skippedは判定コマンドでのみ生じる) |
...
| DOM-execution-008 | RunningDecision 値オブジェクト | spec/domains/execution.md#分類の決定直和型 | `Judge(ExitCode)`/`KeepRunning`/`KillOnTimeout`/`DiedWithoutExit`の4値でrunning分類結果を表現する |
...
| DOM-execution-016 | InconsistentRunFiles エラー型 | spec/domains/execution.md#launchingclassifier | ラッパーの書き込み順序保証が破れたケースをmessage付きで表現し、当該tickはスキップして次tickで再観測させる |
| DOM-execution-017 | RunningClassifier.classify_alive ドメイン関数 | spec/domains/execution.md#runningclassifier | exit Someなら観測なしで即`Judge`、exit Noneなら生存観測後`Alive`かつtimeout未超過は`KeepRunning`、超過は`KillOnTimeout`、`Dead`は`DiedWithoutExit`を返す(timeoutはstarted_wall起点) |
...
| DOM-execution-019 | JudgementService.default_judgement ドメイン関数 | spec/domains/execution.md#judgementservice | 0=`Completed`、非0=`Failed`の2値判定(Skippedは返さない) |
...
| DOM-execution-022 | NotificationService.notify_env ドメイン関数 | spec/domains/execution.md#notificationservice | `TASK_ID`/`WORKFLOW`/`TASK_STATUS`の3変数を構成し、notify_cmd未定義時は通知せず`notified_at`も書かない契約を担う |
...
| DOM-execution-041 | RunStore.write_starttime ポートメソッド | spec/domains/execution.md#runstore | アトミック置換で`StartTimeRecord`を書き、書きかけを観測させない |
| DOM-execution-042 | RunStore.write_pid_file ポートメソッド | spec/domains/execution.md#runstore | アトミック置換で`PidFileContent`を書き、書きかけを観測させない |
| DOM-execution-043 | RunStore.write_exit ポートメソッド | spec/domains/execution.md#runstore | アトミック置換で`ExitCode`を書き、書きかけを観測させない |
...
| DOM-definition-055 | WorkflowLoadError エラー型 | spec/domains/definition.md#workflowstore | `NotFound{attempted}`(解決を試みた絶対パス) / `Parse(WorkflowParseError)` / `Io{message}` の3種を区別する |
...
| DOM-execution-071 | NotificationService ドメインサービス | spec/domains/execution.md#notificationservice | 組み込み `NOTIFY_TIMEOUT=60秒`(ADR-018)を notify_cmd に必ず適用する。超過・起動不能・非0終了はいずれも通知失敗として `notified_at` を書かず次tickの再通知に委ねる |
